        ...
        public static void Ensure()
        {
            ...
            try
            {
                foreach (TMP_SpriteAsset sa in Resources.FindObjectsOfTypeAll<TMP_SpriteAsset>())
                {
                    ...
                    _registered = true;
                    try { SellValuePlugin.Log.LogInfo("Gold icon: registered sprite asset '" + AssetName + "' (" + (sa.spriteCharacterTable != null ? sa.spriteCharacterTable.Count : 0) + " sprites)"); } catch { }
                    return;
                }
            }
            catch { }
