...
using HarmonyLib;
using SellValue.Patches;

namespace SellValue
{
    [BepInPlugin(PluginInfo.PLUGIN_GUID, PluginInfo.PLUGIN_NAME, PluginInfo.PLUGIN_VERSION)]
    public class SellValuePlugin : BaseUnityPlugin
    {
        internal static ManualLogSource Log;
        internal static SellValueConfig Cfg;
        ...
        private void Awake()
        {
            ...
            Cfg = new SellValueConfig(Config);
        ...
        {
            if (_patched) return;
            Type[] containers = { typeof(SellPatches) };
        ...
        private void Update()
        {
            if (_patched) { try { SellPatches.Tick(); } catch (Exception e) { if (Cfg != null && Cfg.Verbose.Value) Log.LogWarning("Tick: " + e); } }
