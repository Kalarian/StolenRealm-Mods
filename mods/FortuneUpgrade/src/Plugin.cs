...
using HarmonyLib;
using FortuneUpgrade.Patches;

namespace FortuneUpgrade
{
    [BepInPlugin(PluginInfo.PLUGIN_GUID, PluginInfo.PLUGIN_NAME, PluginInfo.PLUGIN_VERSION)]
    public class FortuneUpgradePlugin : BaseUnityPlugin
    {
        internal static ManualLogSource Log;
        internal static FortuneUpgradeConfig Cfg;
        ...
        private void Awake()
        {
            ...
            Cfg = new FortuneUpgradeConfig(Config);
        ...
        {
            if (_patched) return;
            Type[] containers = { typeof(UpgradePatches) };
        ...
        private void Update()
        {
            if (_patched) { try { UpgradePatches.Tick(); } catch (Exception e) { if (Cfg != null && Cfg.Verbose.Value) Log.LogWarning("Tick: " + e); } }
