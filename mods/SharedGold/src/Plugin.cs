...
using HarmonyLib;
using SharedGold.Patches;

namespace SharedGold
{
    [BepInPlugin(PluginInfo.PLUGIN_GUID, PluginInfo.PLUGIN_NAME, PluginInfo.PLUGIN_VERSION)]
    public class SharedGoldPlugin : BaseUnityPlugin
    {
        internal static ManualLogSource Log;
        internal static SharedGoldConfig Cfg;
        ...
        private void Awake()
        {
            ...
            Cfg = new SharedGoldConfig(Config);
        ...
        {
            if (_patched) return;
            Type[] containers = { typeof(GoldPatches) };
        ...
        private void Update()
        {
            if (_patched) { try { GoldPatches.Tick(); } catch { } }
            ...
            if (reloadKey || MasterConfig.ReloadRequested(ref _reloadSeen))
            {
                ...
                if (_patched) GoldPatches.Consolidate("reload");
