...
namespace ModMenu
{
    ...
    internal static class ModTable
    {
        ...
        private static readonly Dictionary<string, Entry> _entries = new Dictionary<string, Entry>
        {
            ...
            { "BattleStats", new Entry { Title = "Battle stats that add up", Description = "The post-battle Stats window gains a damage breakdown, ticks and tiles, elements, crits, kills and more. A Run Stats button under the gold and difficulty box (or F8) opens the same page with the totals for the whole run since you left town, and every run is saved so you can pull up older ones (History, or F7)." } },
            ...
            { "ItemSkillTooltips", new Entry { Title = "Item skill tooltips", Description = "Hover an item that grants a skill or passive: the skill's own tooltip appears next to the item tooltip, with numbers at the item's level." } },
            { "SellValue", new Entry { Title = "Sell value on item tooltips", Description = "Every item tooltip shows what a shop pays for the item, bottom right, with the stack total and the value of one unit." } },
            { "RoguelikeQoL", new Entry { Title = "Roguelike QoL", Description = "Roguelike only: 4 of 5 event nodes lead to a fortune (vanilla: 1 in 2), and the forced-roll hazard events (traps, storms, rockslides) that punish a failed roll for the whole run are removed from the pool, the meta currency earned is multiplied (1.5x), gold earned by 1.25x, and the gear offered after each battle rolls higher rarities (Rare 25%, Legendary 10%, Mythic 5% instead of 16/3/1). Host decides the events in co-op; currency is per player." } },
