...
using HarmonyLib;
using SpecialTooltips.Patches;

namespace SpecialTooltips
{
    [BepInPlugin(PluginInfo.PLUGIN_GUID, PluginInfo.PLUGIN_NAME, PluginInfo.PLUGIN_VERSION)]
    public class SpecialTooltipsPlugin : BaseUnityPlugin
    {
        internal static ManualLogSource Log;
        internal static SpecialTooltipsConfig Cfg;
        internal static Descriptions Desc;
        ...
        private void Awake()
        {
            ...
            Cfg = new SpecialTooltipsConfig(Config);
            Desc = new Descriptions(System.IO.Path.Combine(Paths.ConfigPath, "stolenrealm.specialtooltips.descriptions.txt"));
            Desc.Load();
        ...
        {
            if (_patched) return;
            Type[] containers = { typeof(ExaminePatches) };
